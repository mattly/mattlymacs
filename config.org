...
** Font
   ...
   #+BEGIN_SRC emacs-lisp
     (defvar mattly-font "Iosevka Light 16")
...
*** Binding Helpers
    ...
    #+BEGIN_SRC emacs-lisp
      ...
      (general-create-definer bind-navigation-for-maps
                              :states '(normal visual motion))

      (general-create-definer bind-modifier
                              :keymaps '(evil-normal-state-map evil-visual-state-map))
      (general-create-definer bind-modifier-for-maps
                              :states '(normal visual))

      (general-create-definer bind-normal
                              :keymaps '(evil-normal-state-map))
      (general-create-definer bind-normal-for-maps
                              :states '(normal))
      (general-create-definer bind-visual
      ...
      (general-create-definer bind-operator-for-maps
                              :states '(operator))
      ...
      (general-create-definer bind-insert
                              :keymaps '(evil-insert-state-map))
      (general-create-definer bind-insert-for-maps
                              :states '(insert))

      (general-create-definer bind-everywhere
                              :keymaps 'global
                              :states '(insert
                                        normal
                                        visual
     ...
     #+BEGIN_SRC emacs-lisp
       (use-package smartparens
         ...
         (setq sp-autoinsert-pair t
               sp-autowrap-region nil ; let others handle this
...
** In-Buffer Navigation
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (evil-define-motion evil-avy-find-char (count char)
       "Use avy to move forward to char."
       :jump t
       :type inclusive
       (interactive "<c><C>")
       (if (null count) (avy-goto-char char)
         (evil-find-char count char)))

     (evil-define-motion evil-avy-find-char-to (count char)
       "Use avy to move till char."
       :jump t
       :type inclusive
       (interactive "<c><C>")
       (if (null count)
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package persp-mode
       ...
       (setq persp-autokill-buffer-on-remove 'kill-weak
             ...
             persp-save-dir (concat mattlymacs-local-dir "persp/")
...
*** Lisp Mode
    #+BEGIN_SRC emacs-lisp
      (defvar evil-lisp-mode-map (make-sparse-keymap))

      (define-minor-mode evil-lisp-mode
        "A minor mode for lisp editing in lisp"
        :init-value nil
        :lighter " (lisp)"
        :keymap evil-lisp-mode-map 
        (parinfer-mode 1))

      (bind-navigation-for-maps
       :keymaps '(evil-lisp-mode-map)
       "N" 'evil-cp-backward-symbol-begin
       "I" 'evil-cp-forward-symbol-begin
       "O" 'evil-cp-forward-symbol-end

       "`" '(parinfer-toggle-mode))

      (bind-modifier-for-maps
       :keymaps 'evil-lisp-mode-map
       "\C-s" 'evil-cp-insert-at-end-of-form
       "\C-r" 'evil-cp-insert-at-beginning-of-form
       "\C-t" 'evil-cp-change-sexp
       "\C-d" 'evil-cp-delete-sexp
       "\C-z" 'evil-cp-open-below-form
       "\C-Z" 'evil-cp-open-below-form
       ...
       "w J" 'sp-join-sexp
       "w S" 'sp-split-sexp
       "w n" 'evil-cp-drag-backward
       "w i" 'evil-cp-drag-forward
       "w TAB" 'sp-indent-defun)

      (define-key parinfer-mode-map [tab] 'parinfer-smart-tab:forward-char)
      (define-key parinfer-mode-map [backtab] 'parinfer-smart-tab:backward-char)
    #+END_SRC
...
*** Navigation
    #+BEGIN_SRC emacs-lisp
      (bind-navigation
       ...
       "k" '(:ignore t :which-key "nav")
       "k k" 'evil-jump-item
       "k u" 'outline-previous-visible-heading
       "k e" 'outline-next-visible-heading
      ...
      (bind-operator
       ...
       "s" evil-outer-text-objects-map
       "t" 'evil-surround-edit
       "T" 'evil-Surround-edit)
...
*** Modification
    #+BEGIN_SRC emacs-lisp
      (bind-modifier
       "r" '(evil-insert :which-key "insert here")
       "R" '(evil-insert-line :which-key "insert at SOL")
       "s" '(evil-append :which-key "append here")
       "S" '(evil-append-line :which-key "append at EOL")
       "t" '(evil-change :which-key "change ______")
       "T" '(evil-change-line :which-key "change to EOL")
       "q" '(evil-replace :which-key "replace char")
       "Q" '(evil-replace-state :which-key "overwrite mode")
       "z" '(evil-open-below :which-key "open below")
       "Z" '(evil-open-above :which-key "open above")
       "d" '(evil-delete :which-key "delete ______")
       "D" '(evil-delete-line :which-key "delete to EOL")

       "a" '(evil-visual-char :which-key "visual char mode")
       "A" '(evil-visual-line :which-key "visual line mode")
       "C-a" '(evil-visual-block :which-key "visual block mode")

       "w" '(:ignore t :which-key "modify")
       "w c" '(evil-commentary :which-key "comment ______")
       "w j" '(evil-join :which-key "join line")
       "w s" '(evil-surround-edit :which-key "surround ______"))

      (bind-normal
       "p" '(undo-tree-undo :which-key "undo")
       "P" '(undo-tree-redo :which-key "redo")

       ";" '(evil-record-macro :which-key "record macro")
       ":" '(lambda ()
              (interactive)
              (evil-execute-macro 1 last-keyboard-macro))
       "'" '(evil-ex :which-key "ex")

       "TAB" '(:ignore t :which-key "fold")
       "TAB TAB" '(origami-recursively-toggle-node :which-key "close")
       "TAB c" '(origami-close-node :which-key "close")
       "TAB C" '(origami-close-node-recursively :which-key "close recursively") 
       "TAB m" '(origami-forward-toggle-node :which-key "toggle")
       "TAB M" '(origami-toggle-all-nodes :which-key "toggle recursively")
       "TAB o" '(origami-open-node :which-key "open")
       "TAB O" '(origami-open-node-recursively :which-key "open recursively")
       "TAB a" '(origami-open-all-nodes :which-key "open all")
       "TAB A" '(origami-close-all-nodes :which-key "close all")
       "TAB e" '(origami-next-fold :which-key "go next")
       "TAB u" '(origami-previous-fold :which-key "go previous"))


      (bind-visual
       "z" '(exchange-point-and-mark :which-key "toggle point")
       "Z" '(evil-visual-exchange-corners :which-key "switch corner (block mode)"))
    #+END_SRC
*** Et Cetera
    #+BEGIN_SRC emacs-lisp
      (bind-everywhere
       "?" '(which-key-show-top-level :which-key "show mappings")
...
*** Top-Level
    #+BEGIN_SRC emacs-lisp
      (bind-main-menu
       "/" '(counsel-projectile-ag :which-key "search in current directory")
...
*** b is for Buffer
    #+BEGIN_SRC emacs-lisp
      (bind-main-menu
       ...
       "b" '(ivy-switch-buffer :which-key "menu ALL")
...
*** f is for Files
    #+BEGIN_SRC emacs-lisp
      (bind-main-menu
       ...
       "d" '(counsel-projectile-find-dir :which-key "find dir in project")
       "f" '(counsel-find-file :which-key "find file")
       "r" '(projectile-recentf :which-key "recent files")
       "R" '(counsel-recentf :which-key "recent files")
       "s" '(save-buffer :which-key "save")
       "SPC k" '(delete :which-key "delete (any file)")
       "SPC R" '(rename-file :which-key "rename (any file)"))
    #+END_SRC
...
*** p is for Project
    #+BEGIN_SRC emacs-lisp
      (bind-main-menu
       ...
       "b" '(counsel-projectile-switch-to-buffer :which-key "menu")
       "f" '(counsel-projectile-find-file :which-key "find file in project")
       "p" '(projectile-switch-project :which-key "switch project")
       "t" '(neotree-toggle :which-key "open neotree")
   ...
   #+BEGIN_SRC emacs-lisp
     (general-define-key
      :keymaps '(minibuffer-local-map
                 ...
                 minibuffer-local-compoetion-map
...
** Clojure
   #+BEGIN_SRC emacs-lisp
     (use-package clojure-mode
       ...
       :config
       (add-hook 'clojure-mode-hook #'evil-lisp-mode))
   #+END_SRC
** Emacs Lisp
   #+BEGIN_SRC emacs-lisp
     (defun mattly|init-emacs-lisp-mode ()
      (evil-lisp-mode 1))

     (add-hook 'emacs-lisp-mode-hook #'mattly|init-emacs-lisp-mode)
   #+END_SRC
** Markdown 
   #+BEGIN_SRC emacs-lisp
...
*** key bindings
   ...
   #+BEGIN_SRC emacs-lisp
     (bind-navigation-for-maps
      ...
      "k i" 'org-forward-heading-same-level
      "k n" 'org-backward-heading-same-level)
     (bind-normal-for-maps
      :keymaps 'org-mode-map
      "RET" 'org-return-indent
      "w i" 'org-demote-subtree
      "w n" 'org-promote-subtree
      "w l" 'org-do-promote
      "w y" 'org-do-demote
      "w u" 'org-move-subtree-up
      "w e" 'org-move-subtree-down)
