...
*** minor-mode key bindings
   #+BEGIN_SRC emacs-lisp
     (evil-define-state org
       "org state. Used to navigate and manipulate the org outline."
       :tag " <OS> "
       :suppress-keymap t
       :cursor (bar . 2))

     (defmacro evil-org-state-enter-command (command)
       "Wrap COMMAND to call evil-org-state before executing COMMAND."
       (let ((funcname (if (string-match "org-state-" (symbol-name command))
			   (intern (format "evil-%s" command))
			 (intern (format "evil-org-state-%s" command)))))
	 `(progn
	    (defun ,funcname ()
	     (interactive)
	     (evil-org-state)
     ...
     (defun evil-org-state/quit ()
       "Quit org state and set state to normal."
       (interactive)
       (evil-normal-state))

     (defun evil-org-state/init ()
       "Initialize org state and display the keymap."
       (interactive)
       (evil-org-state))
   #+END_SRC
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (general-define-key
      ...
      :states '(normal visual operator)
      :keymaps 'org-mode-map
      "k" '(evil-org-state/init :which-key "structure mode (org)"))

     (general-define-key
      :states '(org)
      :keymaps 'org-mode-map
      "M-x" '(counsel-M-x)
      "ESC" '(evil-org-state/quit :which-key "quit to normal")
